...
export function normalizeRow(raw) {
  ...
  return {
    ...
    status: str(pickCol(row, 'card_status')),
    orderStatus: str(pickCol(row, 'order_status')),
...
/**
 * order_url in 4093 is built from the INTERNAL id
 * ('https://admin.arenaclub.com/orders/' || o.id) while order_number is
 * o.number — a different value. Never construct a link from order_number;
 * use the URL the query already returns. Repack, auction-less and purchase
 * rows return NULL, so those order numbers render as plain text.
 */
...
export function buildResult(rawRows) {
  ...
  const card = {
    ...
    lifecycle: lifecycle.map((r) => ({
      ...
      url: isUrl(r.orderUrl) ? r.orderUrl : '',
    })),
  };
